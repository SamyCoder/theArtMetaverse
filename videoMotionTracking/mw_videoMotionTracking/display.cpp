...
Display::FPS::FPS(int size) : prevTick(0), size(size),
    index(0), total(0), sum(0) {
...
void drawTracks(cv::UMat& image, const Tracks& tracks,
        cv::Point& mousePos, double fps) {
    for(const std::unique_ptr<Track>& track : tracks) {
        // Draw contour outline
        const Contour& contour = track->getContour();
        cv::drawContours(image, std::vector<Contour>({contour}),
                0, contourColor, contourThickness);
        // Draw bounding box
        ...
        cv::circle(image, track->getPrediction(), predictionSize,
                predictionColor, -1);
        // Annotate bounding box with track information
        std::ostringstream labelTextStream;
        labelTextStream << "Blob " << track->getId() << 
            " (Area: " << cv::contourArea(contour) << ")";
        std::string labelText = labelTextStream.str();
        // - Measure text size
        cv::Size labelSize = cv::getTextSize(labelText, labelFont,
                labelScale, labelFontThickness, nullptr);
        ...
        cv::putText(image, labelText, labelPos, labelFont, labelScale,
                labelColor, labelFontThickness);
    }
    // Draw FPS text
    std::ostringstream fpsTextStream;
    fpsTextStream << "FPS: " << std::fixed << std::setprecision(2) << fps;
    cv::putText(image, fpsTextStream.str(),
            {0, image.rows - 8}, labelFont, labelScale, fpsColor);
}
...
Display::Display(std::string winTitle) : winTitle(winTitle), fps(100) {
    // Initialize windows
    cv::namedWindow(winTitle);
    cv::setMouseCallback(winTitle, mouseCallback, (void*) &mousePos);
}
...
void Display::showFrame(cv::UMat& frame,
        cv::UMat& maskImage, const Tracks& tracks) {
    // Calculate FPS
    ...
    cv::imshow(winTitle, buffer);
    cv::resizeWindow(winTitle,300,300);
}
